...
/// ## 频繁调用的说明
///
/// 此函数在跨进程锁的获取循环中被调用（最多 30 次，间隔 200ms）。
/// 每次调用都会生成子进程（`kill -0` / `tasklist`），理论上开销不低。
///
/// ### 为什么没有添加 TTL 缓存？
///
/// 添加缓存会引入以下复杂度：
/// - 需要线程安全的静态缓存 + TTL 过期机制
/// - 缓存可能返回过期结果，导致误判残留锁或活锁
/// - 在此场景（最多 30 次/6 秒）下，子进程开销可忽略
///
/// ### 优化方向
///
/// 如果未来需要优化，可以考虑：
/// 1. 在锁获取循环内简单缓存本次结果（不跨重试）
/// 2. 仅在第一次检查到 PID 存活后缓存，避免重复创建子进程
