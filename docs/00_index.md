...
> 面向普通开发者和入门开发者的 blockcell 系列索引，当前共 29 篇，全部基于真实代码与当前实现整理。
...
- CLI、Provider、MCP、Provider Pool、ModelRouter、Hook、intentRouter、多智能体配置等更贴近真实产品落地的专题
...
| 28 | [ModelRouter 智能路由与自动降级](./28_model_router.md) | `routingStrategy`、成本优化、连接阶段降级与多模型策略 | 深入 |
| 29 | [Hook 生命周期事件系统](./29_hook_system.md) | `hooks.yaml`、工具调用前后事件、审计、格式化与通知 | 深入 |
...
**如果你想做真实产品配置：** 02 → 08 → 17 → 18 → 19 → 20 → 28 → 29 → 21

**如果你想理解系统内部设计：** 11 → 12 → 13 → 15 → 16 → 27 → 28 → 29
