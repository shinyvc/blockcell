...
| `preview_size_chars` | 2,000 | usize | 持久化后预览大小（字符数，旧名 `previewSizeBytes` 仍作为兼容别名） | 500 ~ 10,000 |
...
- `preview_size_chars`: 2000 字符预览足够展示文件头/错误信息开头（旧名 `previewSizeBytes` 已标记为兼容别名）
...
| `previewSizeChars` | 持久化后在历史中保留的预览字符数（旧名 `previewSizeBytes` 仍作为兼容别名） | 预览过长，未达到压缩目的 | 预览太短，LLM 无法判断是否需要 `session_recall` | 独立参数 |
  ...
  "memory": {
    ...
    "memorySystem": {
      ...
      "layer1": {
        ...
        "previewSizeChars": 2000,                 // 持久化预览大小（字符数，旧名 previewSizeBytes 仍兼容）
  ...
  "memory": {
    "memorySystem": {
      ...
      "layer1": {
        ...
        "previewSizeChars": 5000,               // 2.5x: 更长的预览帮助判断是否需要完整内容
